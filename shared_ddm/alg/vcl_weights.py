...
def run_vcl_shared_weights(hidden_size, no_epochs, data_gen, coreset_method,
                   coreset_size=0, batch_size=None, ml_init_option=False, no_iters=1, learning_rate=0.005, path = "", epoch_pause=[]):
    ...
    for task_id in range(no_tasks):
        ...
        #np.savez('alg/weights_%d.npz' % task_id, lower=lower_post, upper=upper_post)
        ...
        acc = utils.get_scores(model, x_testsets, y_testsets, no_repeats=1)
        if task_id == 0:
