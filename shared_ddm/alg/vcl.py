...
class FactorManager():
    ...
    def compute_dist(self, dl_idx, cl_idx, task_idx, remove_data, remove_core):
        ...
        for class_ind in range(len(self.du_m)):
            upper_mv.append([deepcopy(self.du_m[class_ind]), deepcopy(self.du_v[class_ind])])

        return (deepcopy(self.dl_m), deepcopy(self.dl_v)), upper_mv
    ...
    def update_factor(self, post_l_mv, post_u_mv, cav_l_n, cav_u_n,
        ...
        for class_ind in range(len(post_u_mv)):
            self.du_m[class_ind] = deepcopy(post_u_mv[class_ind][0])
            self.du_v[class_ind] = deepcopy(post_u_mv[class_ind][1])


def init_post(cav_info, init_using_cav, ml_weights=None):
    if init_using_cav:
        return deepcopy(cav_info)
    else:
