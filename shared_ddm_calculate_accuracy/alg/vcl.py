...
def run_vcl_shared(hidden_size, no_epochs, data_gen, coreset_method,
                   coreset_size=0, batch_size=None, ml_init_option=False, small_init_always=True, init_prev_means_small_var=False, no_iters=1, learning_rate=0.005, epoch_pause=[]):
    ...
    for task_id in range(no_tasks):
        ...
        for i in range(no_iters):
            ...
            if task_id == 0 and i == 0:
                ...
                if small_init_always:
                    lower_post = init_post(lower_mv, init_using_cav=False)
                    lower_transform = log_func
                elif init_prev_means_small_var:
                    lower_post = init_post(lower_mv, init_using_cav=False, ml_weights=lower_post)
                    lower_transform = log_func
                else:
